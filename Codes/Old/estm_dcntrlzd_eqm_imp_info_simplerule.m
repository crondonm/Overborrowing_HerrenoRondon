...
CT = (1+r)*(1+tao)*bbb + exp(gTT) - bbs.*exp(gTT) - bbb*tao*(1+r);
...
while Delta > Conv
    ...
    mu = 1 - ((1+r)*(1+tao)*beta*(exp(gTT).^(-rho)).*reshape(Pi*reshape(Lambda,[Yn bn*bn]),[Yn*bn bn]))./lambda ; % Lagrange multiplier with respect to the collateral
...
IICECCSR.V = V;
IICECCSR.Ytot = Ytot;
IICECCSR.LambdaSim = LambdaSim;
IICECCSR.Pol = Pol;
IICECCSR.UOpt = UOpt;
IICECCSR.CTOpt = CTOpt;
IICECCSR.COpt = COpt;
IICECCSR.S = S; 
IICECCSR.mCT = mCT;
IICECCSR.mCN = mCN;
...
IICECCSR.CA = CA;
IICECCSR.SimB = SimB ;
IICECCSR.Index = Index;
IICECCSR.PSim = PSim;
IICECCSR.CSim = CSim;
IICECCSR.CTSim = CTSim;
IICECCSR.CNSim = CNSim;
IICECCSR.BCSim = BCSim;
IICECCSR.Cycles = Cycles;
IICECCSR.b = b;
IICECCSR.yt = yt;
IICECCSR.yn = yn;
IICECCSR.gt = gt;
...
IICECCSR.AAA = AAA;
IICECCSR.CCC = CCC;
IICECCSR.CrInd = CrInd;
IICECCSR.Freq = Crisis2 
IICECCSR.DtoY = DtoY;
IICECCSR.mDtoY = mean(DtoY)
IICECCSR.CtoY = CtoY;
IICECCSR.mCtoY = mean(CtoY);
IICECCSR.CTtoY = CTtoY;
IICECCSR.CNtoY = CNtoY;
IICECCSR.CAtoY = CAtoY;
IICECCSR.mCTtoY = mean(CTtoY);
IICECCSR.mCNtoY = mean(CNtoY);
IICECCSR.mCAtoY = mean(CAtoY);
IICECCSR.stdZBB = ZBBT;
IICECCSR.stdGBB = GBBT;
IICECCSR.Posterioryt = Posterioryt;
IICECCSR.Posterioryn = Posterioryn;
IICECCSR.Posteriorg = Posteriorg;
...
%fprintf("Done \n")
%save('IICEIRFSR.mat','IICEIRF','-v7.3');
